...
void spp_data_send_task(void * arg)
{
	uint8_t msg_pub[300];
    int msg_len=0;
    ...
    for(;;)
    {
        ...
        if ((is_connected)&&(enable_data_ntf))
			{
//				msg_len = sprintf((char *)msg_pub,"{\"PowerSwitch\":1,\"CurrentTemperature\":%.1f,\"CurrentHumidity\":%d,\"Atmosphere\":%.1f,\"CO2\":%d,\"HCHO\":%.2f,\"PM25\":%d}\n",
//											bmp280_temp.value_put,(int)bmp280_humi.value_put,bmp280_press.value_put,(int)mh_co2.value_get,pms_hcho.value_get,(int)pms_pm25.value_get);

        	    //esp_ble_gatts_send_indicate(spp_gatts_if, spp_conn_id, spp_handle_table[SPP_IDX_SPP_DATA_NTY_VAL],msg_len, (uint8_t *)msg_pub, false);

                if(msg_len <= (spp_mtu_size - 3)){
                	esp_ble_gatts_send_indicate(spp_gatts_if, spp_conn_id, spp_handle_table[SPP_IDX_SPP_DATA_NTY_VAL],msg_len, msg_pub, false);
                }else if(msg_len > (spp_mtu_size - 3)){
                    if((msg_len%(spp_mtu_size - 3)) == 0){
                        total_num = msg_len/(spp_mtu_size - 3);
                    }else{
                        total_num = msg_len/(spp_mtu_size - 3) + 1;
                    }
                    current_num = 1;
                    ntf_value_p = (uint8_t *)malloc((spp_mtu_size-3)*sizeof(uint8_t));
                    if(ntf_value_p == NULL){
...
//                            ntf_value_p[2] = total_num;
//                            ntf_value_p[3] = current_num;
                            memcpy(ntf_value_p,msg_pub + (current_num - 1)*(spp_mtu_size-3),(spp_mtu_size-3));
                            esp_ble_gatts_send_indicate(spp_gatts_if, spp_conn_id, spp_handle_table[SPP_IDX_SPP_DATA_NTY_VAL],(spp_mtu_size-3), ntf_value_p, false);
                        }else if(current_num == total_num){
//                            ntf_value_p[0] = '#';
//                            ntf_value_p[1] = '#';
//                            ntf_value_p[2] = total_num;
//                            ntf_value_p[3] = current_num;
                            memcpy(ntf_value_p,msg_pub + (current_num - 1)*(spp_mtu_size-3),(msg_len - (current_num - 1)*(spp_mtu_size - 3)));
                            esp_ble_gatts_send_indicate(spp_gatts_if, spp_conn_id, spp_handle_table[SPP_IDX_SPP_DATA_NTY_VAL],(msg_len - (current_num - 1)*(spp_mtu_size - 3)), ntf_value_p, false);
                        }
                        vTaskDelay(20 / portTICK_PERIOD_MS);
                        current_num++;
                    }
...
static void spp_task_init(void)
{
    ...
    xTaskCreate(spp_data_send_task, "data_send_task", 2048, NULL, 10, NULL);
}
